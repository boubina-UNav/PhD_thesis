...
valores <- matrix(0,dim(taboa)[1],dim(taboa)[2]-2)
...
for (m in levels(taboa$Adhesive)) {
  ...
  for (l in Rates) {
    ...
    # Plot frequency
    minFreq = min(valores[taboa$Adhesive!="Clarity PHL 4150",colF])
    maxFreq = max(valores[taboa$Adhesive!="Clarity PHL 4150",colF])
    ...
    legend('topright',legend=nivel,col=corLenda,pch=16,bg="transparent",title="Pair (mbar)")
    titulo <- as.character(taboa[indices[1],]$Adhesive)
    titulo <- paste(titulo,", Qadh",l,"g/s")
    title(titulo)
    #Plot diameter
    minDiam = min(valores[taboa$Adhesive!="Clarity PHL 4150",colD])
    maxDiam = max(valores[taboa$Adhesive!="Clarity PHL 4150",colD])
    ...
    legend('bottomright',legend=nivel,col=corLenda,pch=16,bg="transparent",title="Pair (mbar)")
    title(titulo)
  }
}
