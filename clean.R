...
for (i in 1:(length(indNom)-1)) {
  ...
  if (grepl("PHL4150",nome)) {
    Adh[indices] <- rep("Clarity PHL 4150",numero)
    Tadh[indices] <- rep(145,numero)
    if (grepl("Warm",nome)) Tair[indices] <- rep(170,numero)
    else Tair[indices] <- rep(110,numero)
    if (grepl("/Q1/",nome)) Qadh[indices] <- rep(1,numero)
    else if (grepl("/Q2/",nome)) Qadh[indices] <- rep(2.8,numero)
    else Qadh[indices] <- rep(3.9,numero)
  }
  else {
    Tadh[indices] <- rep(155,numero)
    if (grepl("Warm",nome)) Tair[indices] <- rep(180,numero)
    else Tair[indices] <- rep(120,numero)
    if (grepl("/Q1/",nome)) Qadh[indices] <- rep(1,numero)
